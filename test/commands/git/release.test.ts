...
import Release from '../../../src/commands/git/release';
import * as createReleaseModule from '../../../src/modules/apps/git/release/createRelease';

vi.mock('../../../src/modules/apps/git/release/createRelease', () => ({
  release: vi.fn(),
}));
...
describe('Release Command (git release)', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    releaseSpy = vi.spyOn(createReleaseModule, 'release');
  ...
  it('should call release with default flags and args', async () => {
    ...
    await command.run();
    expect(releaseSpy).toHaveBeenCalledWith(expect.objectContaining({ yes: true }), 'beta');
  });
  ...
  it('should handle errors from release', async () => {
    command._mockParseResult = { flags: { yes: true }, args: { tagName: 'beta' } };
    releaseSpy.mockRejectedValueOnce(new Error('fail'));
