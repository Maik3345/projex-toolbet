...
describe('ensureBranchAvailable', () => {
  ...
  it('should throw error when remote branch not found after general fetch', () => {
    mockedRunCommand
      .mockImplementationOnce(() => { throw new Error('not local'); })
      .mockImplementationOnce(() => { throw new Error('not in remote tracking'); })
      .mockImplementationOnce(() => { throw new Error('direct fetch failed'); })
      .mockImplementationOnce(() => '') // general fetch succeeds
      .mockImplementationOnce(() => { throw new Error('still not found'); }); // branch still not available
    
    expect(() => ensureBranchAvailable('main', '/test/path')).toThrow(
      "Branch 'main' not found in remote repository"
    );
  });
  ...
  it('should handle general fetch failure', () => {
    mockedRunCommand
      ...
      .mockImplementationOnce(() => { throw new Error('not in remote tracking'); })
      .mockImplementationOnce(() => { throw new Error('direct fetch failed'); })
      .mockImplementationOnce(() => { throw new Error('general fetch failed'); });
...
describe('getCurrentBranch', () => {
  ...
  it('should throw error when git command fails', () => {
    mockedRunCommand.mockImplementation(() => { throw new Error('git error'); });
...
describe('getChangedFiles', () => {
  ...
  it('should try alternative commands when first fails', () => {
    mockedRunCommand
      .mockImplementationOnce(() => { throw new Error('first failed'); })
  ...
  it('should throw error when all attempts fail', () => {
    mockedRunCommand.mockImplementation(() => { throw new Error('all failed'); });
...
describe('getDetailedCommits', () => {
  ...
  it('should return detailed commit information', () => {
    mockedRunCommand.mockReturnValue('abc123|feat: add feature|feat: add new feature\n\nDetailed description\n\ndef456|fix: bug fix|fix: resolve critical bug\n\nFix description');
...
describe('buildAnalysisContext', () => {
  ...
  it('should build complete analysis context', () => {
    mockedRunCommand
      ...
      .mockReturnValueOnce('abc123|feat: add feature|feat: add new feature\n\ndef456|fix: bug fix|fix: resolve bug\n\n'); // getDetailedCommits
