...
describe("login", () => {
  ...
  it("should save the credentials to the config file", async () => {
    ...
    const serviceGetAuthMock = serviceGetAuth as jest.MockedFunction<
      typeof serviceGetAuth
    >;
  ...
  it("should log an error message and exit if no token information is found", async () => {
    ...
    await expect(
      login(account, email, workspace, apiKey, apiToken)
    ).rejects.toThrowError("process.exit() was called.");
